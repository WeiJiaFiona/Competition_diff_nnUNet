...
def visualize_mask(mask):
    slices = [mask.shape[0] // 2, mask.shape[1] // 2, mask.shape[2] // 2]
    fig, axes = plt.subplots(1, 3, figsize=(15, 5))

    axes[0].imshow(mask[slices[0], :, :], cmap='gray')
    axes[0].set_title('Sagittal Slice')

    axes[1].imshow(mask[:, slices[1], :], cmap='gray')
    axes[1].set_title('Coronal Slice')

    axes[2].imshow(mask[:, :, slices[2]], cmap='gray')
...
if __name__ == '__main__':
    args = Args()
    mask = generate_mask(32, 32, 32, args)
    if mask is not None:
        visualize_mask(mask)