...
def random_select(mask_scan, organ_type):
    ...
    if not np.any(mask_scan):
        raise ValueError("mask_scan does not contain any organ.")
...
def get_fixed_geo(mask_scan, tumor_type, organ_type, args):
    ...
    if tumor_type == 'small':
        ...
        for _ in range(num_tumor):
            # Small tumor
            x = random.randint(int(0.3*medium_radius), int(0.9*medium_radius))
            y = random.randint(int(0.2*medium_radius), int(1*medium_radius))
            z = random.randint(int(0.2*medium_radius), int(1*medium_radius))
            sigma = random.randint(1, 2)
            # geo = get_ellipsoid(x,y,z)
            geo = generate_mask(x, y, z, args)
...
def synthesize_early_tumor(ct_volume, organ_mask, organ_type, vqgan, tester, args):
    device=ct_volume.device

    # generate tumor mask
    tumor_types = ['tiny', 'small']
    tumor_probs = np.array([0, 1])
    total_tumor_mask = []
    ...
    with torch.no_grad():
        ...
        for bs in range(organ_mask_np.shape[0]):
            ...
            tumor_mask = get_fixed_geo(organ_mask_np[bs,0], synthetic_tumor_type, organ_type, args)
