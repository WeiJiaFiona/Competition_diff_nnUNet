    ...
    def __call__(self, data):
        ...
        if 'kidney_label' in data_name or 'liver_label' in data_name or 'pancreas_label' in data_name:
            ...
            while 1:
                ...
                has_label_2 = (d_crop[0]['label'] == 2).any()

                if pixel_num > self.R2voxel(self.fg_thresh) and not has_label_2:
                    break
                if flag > 5 and pixel_num > self.R2voxel(max(self.fg_thresh - 5, 5)) and not has_label_2:
                    break
                if flag > 10 and pixel_num > self.R2voxel(max(self.fg_thresh - 10, 5)) and not has_label_2:
                    break
                if flag > 15 and pixel_num > self.R2voxel(max(self.fg_thresh - 15, 5)) and not has_label_2:
                    break
                if flag > 20 and pixel_num > self.R2voxel(max(self.fg_thresh - 20, 5)) and not has_label_2:
                    break
                if flag > 25 and pixel_num > self.R2voxel(max(self.fg_thresh - 25, 5)) and not has_label_2:
                    break
                if flag > 100:
                    print(f"Unable to find a suitable crop after 100 attempts for {data_name}")
                    break
        else:
            d_crop = super().__call__(d)
...
def main_worker(gpu, args):
    ...
    # if args.distributed:
    #     torch.multiprocessing.set_start_method('spawn', force=True) #in new Pytorch/python labda functions fail to pickle with spawn
    ...
    for line in open(os.path.join(datafold_dir, 'real_{}_val_{}.txt'.format(tumor_type, fold))):
        name = line.strip().split()[1].split('.')[0]
        print("1111111111111111111111111111111111111",name)
        val_img.append(data_root + line.strip().split()[0])
        val_lbl.append(data_root + line.strip().split()[1])
#        val_pseudo_lbl.append('D:/projects/DiffTumor/organ_pseudo_swin_new/{}/'.format(organ_type) + os.path.basename(line.strip().split()[1]))
