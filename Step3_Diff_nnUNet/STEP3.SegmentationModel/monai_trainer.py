...
from TumorGeneration.mask_generation import generate_mask
import random
def train_epoch(model, loader, optimizer, scaler, epoch, loss_func, args, tumor_size_list):
    ...
    output_dir="D:/projects/synthetic_tumor/"
        ...
        for bs in range(data.shape[0]):
            ...
            if 'kidney_label' in data_name or 'liver_label' in data_name or 'pancreas_label' in data_name:
                if random.random() > sample_thresh:
                    ...
                    tumor_probs = np.array([1, 0, 0])
                    synthetic_tumor_type = np.random.choice(tumor_types, p=tumor_probs.ravel())
                    if synthetic_tumor_type == 'early':
                        synt_data, synt_target = synthesize_early_tumor(healthy_data, healthy_target, args.organ_type, vqgan, early_sampler, args)
                    ...
                    # 保存synt_data和synt_target
                    save_synthetic_data(synt_data[0], synt_target[0], output_dir, data_name, epoch, idx, bs)
...
def save_synthetic_data(synt_data, synt_target, output_dir, data_name, epoch, idx, bs):
    # 确保 synt_data 和 synt_target 是四维的
    print("synt_data.ndim: ", synt_data.ndim, "synt_target.ndim", synt_target.ndim)

    if synt_data.ndim != 4 or synt_target.ndim != 4:
        raise ValueError("synt_data and synt_target must be 4-dimensional with shape [1, depth, height, width]")

    # 去掉批次维度以确保保存为 Nifti 文件时是三维的
    synt_data = synt_data[0]
    synt_target = synt_target[0]

    # 打印synt_data和synt_target的形状以确认它们是三维的
    print(f"Saving data: synt_data shape: {synt_data.shape}, synt_target shape: {synt_target.shape}")
    if synt_data.ndim != 3 or synt_target.ndim != 3:
        raise ValueError("After removing batch dimension, synt_data and synt_target must be 3-dimensional")

    # 创建唯一文件名
    base_name = os.path.basename(data_name).replace('.nii.gz', '')
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    data_filename = f"{base_name}_synt_epoch{epoch}_idx{idx}_bs{bs}_{timestamp}_data.nii.gz"
    target_filename = f"{base_name}_synt_epoch{epoch}_idx{idx}_bs{bs}_{timestamp}_target.nii.gz"

    data_path = os.path.join(output_dir, data_filename)
    target_path = os.path.join(output_dir, target_filename)

    # 将synt_data和synt_target保存为Nifti文件
    synt_data_nifti = nib.Nifti1Image(synt_data.cpu().numpy(), affine=np.eye(4))
    synt_target_nifti = nib.Nifti1Image(synt_target.cpu().numpy(), affine=np.eye(4))

    nib.save(synt_data_nifti, data_path)
    nib.save(synt_target_nifti, target_path)

    print(f"Saved data to {data_path} and target to {target_path}")
...
def run_training(model,
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if args.rank == 0 and args.logdir is not None and args.save_checkpoint:
            save_checkpoint(model, epoch, args, best_acc=val_acc_max, filename=f'model_epoch_{epoch}.pt')
            if b_new_best:
